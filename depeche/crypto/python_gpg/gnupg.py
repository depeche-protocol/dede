...
This version also supports the "--hidden-recipient" argument in order
to make discerning the intended recipient harder.
...
def _make_binary_stream(s, encoding):
    ...
    if _py3k:
        ...
        if type(s) is not str:
            buf = s.encode(encoding)
    return _make_memory_stream(buf)
    ...
    def encrypt_file(self, file, recipients, sign=None,
        ...
        if symmetric:
            ...
            for recipient in recipients:
                if hidden_recipient:
                    args.extend(['--hidden-recipient', no_quote(recipient)])
                else:
